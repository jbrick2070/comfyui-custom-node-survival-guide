...
An AI-agent QA harness for ComfyUI custom-node authoring. v2.1 — 349 bible entries + LLM round-robin consult addon.
...
the 349 parsed rules or executable checks. Producer repair and the matching
...
1. **`BUG_BIBLE.yaml` + `tests/bug_bible_regression.py`** — a 349-entry, machine-readable bug bible plus an automated pytest suite that turns the bible's `verify` fields into executable assertions. Point the suite at any custom-node pack and get a pass/fail report in under 2 seconds. No ComfyUI runtime, no model downloads, no manual grepping.
...
349 bible entries across 12 phases; the pytest suite encodes the static-
...
| 02 Environment & Dependencies   | UTF-8 no BOM, no mojibake, no zero-byte files; boot launchers force UTF-8 stdio; SD 1.5 .ckpt offline-load; model-architecture admission and split-revision HF caches | 02.11, 02.12, 02.14–02.16 |
| 03 Registration & Loading       | Isolated per-node loading, namespaced IDs, no ghost registrations; import load-contract, single-instance pack loading | 03.01, 03.03, 03.05, 03.06, 12.23 |
| 04 INPUT_TYPES & Widgets        | Widget positional stability; workflow JSON integrity; preserved-vs-stripped auto-sense; socket-only types; socket-vs-FUNCTION signature contract | 04.01, 04.02, 04.07–04.14 |
| 05 Execution Model              | Coordination, migration, list outputs, interrupts, completion checks; feature-flag/role-policy decoupling; V3 NodeOutput migration, python-scoping traps | 05.05, 05.06, 05.08, 05.09, 05.12–05.14 |
| 06 Caching & IS_CHANGED         | Stale outputs, signature stability, leaks; model-platform empirical compat; deterministic-replay discipline | 06.01–06.07 |
| 07 Tensors, Audio, Video        | VRAM, dtype, audio contracts, motion-onset pad, sample-rate, composite layer ordering; native-resolution ceilings, engine-prompt coupling, prompt budget and palette hygiene; scroll span vs canvas allocation, legibility through a lighten-only blend | 07.01–07.32 |
...
| 11 LLM-Specific                 | Token budgets, prompt-detector contracts, format normalisers, three-tier resilience, typed repair ladders with captured interrupted output, schema-grammar compatibility, shared native termination contracts, cleanup and sparse-rewrite conservation, applied source-grounded scene corrections and current cache receipts | 11.01–11.65, 12.100 |
